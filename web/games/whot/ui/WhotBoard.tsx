...
import { Pause, Play, Home, RotateCcw, X, Volume2, VolumeX } from 'lucide-react';
import gsap from 'gsap';
import { whotSounds, getSoundForCard } from '../logic/sounds';
...
const WhotBoard: React.FC<WhotBoardProps> = ({ playerName = 'Bruh', difficulty = 'medium', onGameOver }) => {
  ...
  const processSequentialDraws = async () => {
    ...
    while (player.cardsToDraw > 0) {
      ...
      whotSounds.play('cardPick'); // Sound for each card picked
      forceUpdate();
  ...
  useEffect(() => {
    if (game.gameOver) {
      ...
      if (game.winner?.name === playerName) {
        whotSounds.play('victory');
      } else {
        whotSounds.play('defeat');
      }
    ...
    if (game.currentPlayerIndex !== 0 && !game.gameOver && !isDrawing && !isPaused && !isDealing && !drawingLockRef.current) {
      ...
      const timer = setTimeout(async () => {
        ...
        if (playableCards.length > 0) {
          ...
          // Play sound for AI card
          const specialSound = getSoundForCard(cardToPlay.number);
          if (specialSound) {
            whotSounds.play(specialSound);
          } else {
            whotSounds.play('cardPlay');
          }
  ...
  const executePlayCard = async (card: WhotCard, chosenShape: WhotShape | null = null) => {
    ...
    if (success) {
      ...
      const specialSound = getSoundForCard(card.number);
      if (specialSound) {
        whotSounds.play(specialSound);
      } else {
        whotSounds.play('cardPlay');
  ...
  return (
    <div className="relative w-full h-full min-h-screen bg-[#24150e] overflow-hidden flex flex-col select-none antialiased font-sans">
      ...
      <div className="absolute top-4 sm:top-6 right-4 sm:right-6 z-[200] flex items-center space-x-3">
        <button
          onClick={() => {
            const newState = !soundEnabled;
            setSoundEnabled(newState);
            whotSounds.setEnabled(newState);
          }}
