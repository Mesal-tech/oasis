...
export const RewardsScreen = () => {
  ...
  const dailyRewards = [
    ...
    { day: 4, reward: '100 Gold', status: 'ready', isToday: true },
  ...
  return (
    <div className="min-h-screen bg-[#09090B] text-white px-4 sm:px-6 lg:px-8 py-6 lg:py-8 overflow-y-auto font-sans">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-start mb-8 lg:mb-12 gap-6">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold mb-1">Rewards</h1>
              <p className="text-[#A1A1AA] text-sm lg:text-base">Track progress, claim gifts, and level up.</p>
            </div>
          </div>
        ...
        {/* Tabs */}
        <div className="border-b border-[#27272A] mb-8 lg:mb-12">
          <div className="flex gap-8 overflow-x-auto pb-4 -mx-4 px-4 lg:mx-0 lg:px-0 no-scrollbar">
            {TABS.map(tab => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`pb-4 text-xs font-bold tracking-widest transition-all relative whitespace-nowrap ${activeTab === tab.id ? 'text-[#FF5D2E]' : 'text-[#71717A] hover:text-white'
                  }`}
              >
                {tab.label}
                {activeTab === tab.id && (
        ...
        <div className="pb-20">
            ...
            <div className="mb-16">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-7 gap-4">
                {dailyRewards.map((day, idx) => {
                  ...
                  const cardBg = isToday ? '#18181B' : '#121215';
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {achievements.map((ach) => (
                  <div key={ach.id} className="bg-[#121215] border border-[#27272A] rounded-xl p-6 hover:border-[#3F3F46] transition-all">
                    ...
                    <p className="text-[#71717A] text-sm mb-6 line-clamp-3">{ach.desc}</p>
                    ...
                    <div className="flex justify-between items-center pt-4 border-t border-[#27272A]">
                      ...
                      {ach.status === 'completed' ? (
                        ...
                        <button disabled className="px-4 py-1.5 bg-[#27272A] text-[#71717A] text-xs font-bold rounded cursor-not-allowed border border-white/5">
                          CLAIM
                        </button>
                      )}
                    </div>
          ...
          {/* Season Pass Section */}
          {activeTab === 'season' && (
            <div className="mt-8 lg:mt-16 bg-[#121215] border border-[#27272A] rounded-2xl p-6 lg:p-8">
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 lg:mb-12 gap-6">
                ...
                <button className="px-6 py-2.5 border border-[#FF5D2E] text-[#FF5D2E] font-bold text-sm rounded-lg hover:bg-[#FF5D2E]/10 transition-colors flex items-center gap-2 w-full lg:w-auto justify-center">
                  <Crown size={16} /> UPGRADE TO PREMIUM
                </button>
              </div>

              {/* Timeline */}
              <div className="relative pt-10 pb-4 px-4 lg:px-10 overflow-x-auto">
                <div className="min-w-[800px] lg:min-w-0"> {/* Ensures horizontal scroll on mobile */}
                  ...
                  <div className="flex justify-between relative z-10">
                    {/* Nodes preserved exactly as original */}
