...
export const ArenaScreen = () => {
  ...
  return (
    <div className="min-h-screen bg-[#09090B] text-white px-4 sm:px-6 lg:px-8 py-6 lg:py-8 overflow-y-auto">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 lg:gap-12">
        ...
        <div className="flex-1">
          ...
          <div className="mb-8 lg:mb-10">
            ...
            <h1 className="text-3xl sm:text-4xl font-bold mb-3">Competitive Arenas</h1>
            <p className="text-[#A1A1AA] text-sm sm:text-base max-w-xl">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 no-scrollbar w-full sm:w-auto">
              {filters.map((filter) => (
                <button
                  key={filter.id}
                  onClick={() => setSelectedFilter(filter.id)}
                  className={`px-5 py-2.5 rounded-full text-xs font-bold whitespace-nowrap transition-all min-w-fit ${selectedFilter === filter.id
            ...
            <button className="flex items-center gap-2 px-5 py-2.5 bg-[#18181B] border border-[#FF5D2E] text-[#FF5D2E] text-xs font-bold rounded-lg hover:bg-[#FF5D2E]/10 transition-colors whitespace-nowrap">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            {tournaments.map((tourney) => (
              <div
                key={tourney.id}
                className="bg-[#121215] border border-[#27272A] rounded-xl p-6 group hover:border-[#3F3F46] transition-all duration-300 cursor-pointer"
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="text-[#71717A] text-[10px] font-bold uppercase tracking-wider flex items-center gap-2">
                    {tourney.type}
                  </div>
                  <div className={`text-[10px] font-bold px-2 py-1 rounded ${tourney.status === 'LIVE'
                    ? 'text-black bg-[#FF5D2E]'
                    : tourney.status === 'FILLING FAST'
                      ? 'text-white bg-[#FF5D2E]/80'
                ...
                <h3 className="text-xl font-bold mb-6 line-clamp-2">{tourney.title}</h3>

                <div className="flex flex-col sm:flex-row justify-between mb-6 pb-6 border-b border-[#27272A] gap-6 sm:gap-0">
                  <div>
                ...
                <div className="w-full h-2 bg-[#27272A] rounded-full overflow-hidden mb-6">
                  <div
                    className={`h-full rounded-full transition-all duration-700 ${tourney.filled >= 90 ? 'bg-red-500' : 'bg-[#FF5D2E]'}`}
                ...
                <div className="flex items-center justify-between">
                  <div className="text-[#71717A] text-xs font-bold flex items-center gap-2">
                    {tourney.status === 'LIVE' ? (
                      <Clock size={14} className="animate-pulse text-[#FF5D2E]" />
                    ) : (
                      <Clock size={14} />
                    )}
                  ...
                  {tourney.locked ? (
                    ...
                    <button className="px-6 py-2.5 border border-[#27272A] bg-[#18181B] text-white text-xs font-bold rounded-lg hover:border-[#FF5D2E] hover:bg-[#FF5D2E]/10 hover:text-[#FF5D2E] transition-all">
                      Join Arena
                    </button>
                  )}
                </div>
        ...
        {/* Sidebar - Featured Tournament (Desktop Only) */}
